...
void MX_FDCAN2_Init(void)
{
  ...
  sFilterConfig.FilterConfig = FDCAN_FILTER_TO_RXFIFO0; // 过滤后到FIFO0
  sFilterConfig.FilterID1 = 0x00000000;          // 过滤ID=0x00
  sFilterConfig.FilterID2 = 0x00000000;          // 掩码=0x00（不过滤）
  if (HAL_FDCAN_ConfigFilter(&hfdcan2, &sFilterConfig) != HAL_OK)
  {
    Error_Handler();
  }

  // 启用FIFO0消息挂起中断（对应原CAN_ITConfig(CAN1, CAN_IT_FMP0, ENABLE)）
  if (HAL_FDCAN_ActivateNotification(&hfdcan2, FDCAN_IT_RX_FIFO0_NEW_MESSAGE, 0) != HAL_OK)
  {
    Error_Handler();
  }
  HAL_FDCAN_ConfigGlobalFilter(&hfdcan2,
  FDCAN_REJECT,              // 拒绝标准帧（仅用扩展帧）
  FDCAN_ACCEPT_IN_RX_FIFO0,  // 接收所有扩展帧到FIFO0（匹配例程）
...
void can_filter_init()
{
  ...
  fdcan_filter.FilterID1 = 0x000;                                 // 接收所有标准ID
  fdcan_filter.FilterID2 = 0x000;                                // 掩码：接收所有帧

  HAL_FDCAN_ConfigFilter(&hfdcan1, &fdcan_filter);

  // 配置全局过滤器 - 接收所有标准帧
  HAL_FDCAN_ConfigGlobalFilter(&hfdcan1,
    FDCAN_ACCEPT_IN_RX_FIFO0,  // 接收所有标准帧到FIFO0
    FDCAN_REJECT,              // 拒绝扩展帧
    FDCAN_REJECT_REMOTE,       // 拒绝远程帧
    FDCAN_REJECT_REMOTE);      // 拒绝远程帧
